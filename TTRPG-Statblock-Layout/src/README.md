...
### Installation
You don't need to install the `.js` files unless you're looking to customize the layout, the skill names, or some other function of the rendering.  By default, if you install the `TTRPG-Statblock-Layout/jsons/SWRPG.json` you'll get a format that works.  If you do wish to customize something, start by installing the json, then update the `.js` files in the Fantasy Statblock layout configuration.

### Dice Rendering
`Statblock-SWRPG-Skills.js` includes variables for Star Wars Ability and Proficiency dice.  By default, these dice are rendered as tags, and take advantage of the Dice-Snippets feature.  The dice tags must match the tags you're using in your Dice-Snippets - for myself I have removed the StarWars prefix, so Ability dice are tagged #Ability and Proficiency dice are tagged #Proficiency.  You can customize this by editing the `abilityDiceTag` and `proficiencyDiceTag` variables in `Statblock-SWRPG-Skills.js` Note that this uses the StarWarsDice-Alt.css snippet (see /Dice-Snippets/snippets/StarWarsDice).
...
### Skill Labels and Wiki Links
By defaults the skill names in `skills` are used as labels for the skill output in the statblock.  Note that the code currently has each skill with a wiki link, surrounded by double square-brackets.  This allows for easy reference to the skill name and assumes you have a page for each skill in your Obsidian vault.  Edit as necessary for your own custom use.
